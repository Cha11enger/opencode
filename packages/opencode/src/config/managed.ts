...
const MANAGED_PLIST_DOMAIN = "ai.wyzord.managed"

// Keys injected by macOS/MDM into the managed plist that are not WYZORD config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/wyzord"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "wyzord")
    default:
      return "/etc/wyzord"
  }
}
