...
import wyzord from "./theme/wyzord.json" with { type: "json" }
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "wyzord")
        draft.active = active === "opencode" ? "wyzord" : typeof active === "string" ? active : "wyzord"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.wyzord, store.mode)
    })
