...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link when sharing is configured",
  ...
  "Create {highlight}wyzord.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/wyzord/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.wyzord/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.wyzord/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.wyzord/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.wyzord/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent WYZORD CLI from reading sensitive files",
  "Use {highlight}wyzord run{/highlight} for non-interactive scripting",
  "Use {highlight}wyzord --continue{/highlight} to resume the last session",
  "Use {highlight}wyzord run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}wyzord serve{/highlight} for headless API access to WYZORD CLI",
  "Use {highlight}wyzord run --attach{/highlight} to connect to a running server",
  "Run {highlight}wyzord upgrade{/highlight} to update to the latest version",
  "Run {highlight}wyzord providers list{/highlight} to see all configured providers",
  "Run {highlight}wyzord agent create{/highlight} for guided agent creation",
  "Use {highlight}/wyzord{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}wyzord github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/wyzord fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.wyzord/themes/{/highlight} directory",
  ...
  "Run {highlight}wyzord debug config{/highlight} to troubleshoot configuration",
  ...
  "Run a WYZORD CLI container image when one is published for your environment",
  "Use {highlight}/connect{/highlight} with WYZORD Cloud for curated, tested models",
