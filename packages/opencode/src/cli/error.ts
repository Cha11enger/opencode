...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, WYZORD CLI does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`wyzord models\` to list available models`,
      `Or check your config (wyzord.json) provider/model names`,
    ].join("\n")
  }
